...
1. **Poll the calendar** via the available MCP server `phils-outlook` to retrieve all calendar events for the current date.
...
6. **Link people** from the `@people/` folder where matches exist; use previous rules for notes in this folder.

## Meeting Note Creation Rules

- IMPORTANT: always use your context file `obsidian-dk-vault.md` to learn about vault conventions, e.g. linking people, etc.
- IMPORTANT: always get the latest version of the vault by first doing a `git pull` on the folder.
- IMPORTANT: after creation, commit and push the changes to git, notifying user this was done.
- do not use characters in filenames that are invalid in either Linux or Windows environments, e.g. "<", ">", etc.
...
- File name format: `YYYY-MM-DD <Person's First Name> 1-1.md`
