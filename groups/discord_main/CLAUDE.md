...
| **LiteLLM — Models** *(Phil's personal LiteLLM proxy instance)* | |
| add-model | Add any LLM provider (OpenAI, Azure, Anthropic, Bedrock, Ollama…) to the proxy |
...
| **LiteLLM — Keys** | |
| add-key | Generate an API key scoped to user, team, budget, and expiry |
